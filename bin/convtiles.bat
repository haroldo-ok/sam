...
for %%f in (*.png) do %BMP2TILEPATH%bmp2tile %%f -palsms -savetiles %%~dpnf.tiles.psgcompr -savetilemap %%~dpnf.tilemap.bin -savepalette %%~dpnf.palette.bin -exit

rem Renames the files to their final names (Yeah, REN alone won't cut it when there's composite file extensions...)
for /f "delims=." %%f in ('dir /b *.tiles.psgcompr') do ren "%%f.tiles.psgcompr" "%%f (tiles).pscompr"
